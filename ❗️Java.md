...
## 🅱️ Meta Space و Perm Gen(Permanent Generation)
...
## 🅱️ عملگر Instanceof
 این عملگر مشخص می‌کند که آیا شیء از نوع یک کلاس می‌باشد یا خیر
اگر شیء از جنس آن کلاس باشد(نمونه‌ای از آن کلاس) مقدار True و در غیر اینصورت همواره مقدار False را در خروجی برمی‌گرداند.
...
public class Person {
 ...
 System.out.println(p2.equals(p3));☓️️️ return False
 }
}
...
String str3 = "Ali";
String str4 = "Ali";
→ str1 == str2 // ☓️️️ مساوی نیستند زیرا هرکدام به یک شیء جدید اشاره دارد
→ str2 == str3 // ☓️️ مساوی نیستند
→ str3 == str4 // ✓ باهم مساوی هستند

ّInteger int1 = new ّInteger(2);
ّInteger int2 = new ّInteger(2);
ّInteger int3 = "Ali";
ّInteger int4 = "Ali";
→ int1 == int2 ☓️️ مساوی نیستند زیرا هرکدام به یک شیء جدید اشاره دارد
→ int2 == int3 ☓️️ مساوی نیستند		#autoboxing
→ int3 == int4 ✓ باهم مساوی هستند	#autoboxing
...
### ✅️ پوشه‌بندی پروژه‌ها
 پوشه‌src : شامل مواردی نظیر: Servlet ، سرویس‌ها،‌ بخش لاجیک کد نوشته شده، انضمام لایه کنترلر
...
Package access (دسترسی در بسته):از سایر کلاس‌های همان بسته هم قابل استفاده است
Private (خصوصی): فقط در همان کلاس قابل استفاده است (از دید هر کلاس دیگری مخفی است)
...
هر متغیر final (در هر دو حالت) باید بلافاصله مقداردهی شود که این مقداردهی می‌تواند بصورت inline یا Initiator Block یا تابع‌سازنده باشد
 ...
 Primitive Variable: اگر از نوع final تعریف شود آنگاه
 ...
 Reference Variable : اگر از نوع final تعریف شود آنگاه
...
final int I = 2;
I = 3;☓️️ # مقدار متغیرهایی از انواع اولیه غیرقابل تغییر است

final Person p1 = new Person();
p1 = p2;☓️️ # هویت یک شیء ثابت قابل تغییر نیست
p1 = new Person();☓️️ # هویت یک شیء ثابت قابل تغییر نیست
p1.setName("Ali");✓ #وضعیت(ویژگی، محتوی) یک شیء ثابت قابل تغییر است
...
ثابت بودن درباره ثبات هویت یا identity شیء است و با final مشخص می‌شود ولی تغیییر ناپذیر بودن درباره ثبات وضعیت (state) است
 ...
 Mutable Object یا اشیاء تغییرپذیر
 ...
 Immutable Object یا اشیاء تغییرناپذیر
...
shape s = new Rectangle();
Circle c = (Circle) s;☓️️ #خطای زمان اجرا
...
c = d; ☓️️ خطای کامپایل
c = (Cat) x;
d = (Dog) x; ☓️️ خطا ClassCastException در زمان اجرا
...
interface A{ void f(); }
interface B{ int f(); }
abstract class C implements A,B{} //☓️️غلط است بدلیل تضاد اسامی
...
# 🅰️ Data Structure and Containers [ساختمان‌داده‌وظرف‌ها]
...
limitation:
 students.add("Taghi Taghavi");	// ☓️️ نمی‌توانیم رشته اضافه نماییم و فقط باید کلاس استیودنت اضافه گردد
 students.add(new Object());		// ☓️️ نمی‌توانیم آبجکت اضافه نماییم و فقط باید کلاس استیودنت اضافه گردد 
...
System.out.println(list.size());	// OUTPUT: 5
 ...
 با تغییر حتی روی یک متد در Container ها سبب «غیرمعتبر» نمودن تمامی iterator های آن Container می‌شود و درادامه هر عملیات روی iterator های غیرمعتبر شده سبب پرتاب خطای Concurrent Modification Exception می‌شود. به این تکنیک، «شکست‌سریع» یا Fail Fast گفته می‌شود
...
String s = itr.next();	//☓️️ سبب بروز خطا می‌شود زیرا «آی‌تی‌آر» نامعتبر است
...
for (String s : list)
 if(s.equals("A"))
 list.remove(s);	//☓️️ با تغییریافتن کالکشن «لیست» ایتریتورهای دیگری که بر روی لیست ساخته شده‌اند نامعتبر می‌شوند
...
List<Integer> ints;	// ✓
List<Student> stus;	// ✓
List<int> error1;	// ☓️️ خطای کامپایل
List<double> error2;	// ☓️️ خطای کامپایل
...
// Sample2
 ...
 Integer p2 = st2.pop();
 st2.push("A");	// ☓️️ در لیست اعداد حروف جای نمی‌گیرد و تولید ارور می‌نماید
...
NumbersQueue<Integer> n;	// ✓ باید عدد باشد پس صحیح است
NumbersQueue<Double> d;		// ✓ باید عدد باشد پس صحیح است
NumbersQueue<String> s;		// ☓️️ باشد عدد باشد ولی رشته گرفته پس غلط است
NumbersQueue<Person> p;		// ☓️️ باید عدد باشد ولی کلاس گرفته پس غلط است
SortedList<Long> l;		// ✓ باید مواردی باشد که «کامپربل» باشد و نوع لانگ هست
SortedList<Float> f;		// ✓ باید مواردی باشد که «کامپربل» باشد و نوع اعشاری هست
SortedList<String> s;		// ✓ باید مواردی باشد که «کامپربل» باشد و نوع رشته هست
SortedList<Person> p;		// ☓️️ باید مواردی باشد که «کامپربل» باشد و نوع کلاس نیست
...
queue.enqueue("Ali");	// ☓️️ رشته نمی‌تواند بگیرد و فقط باید از نوع اعداد باشد
...
Stack<String>[] s = new Stack<String>[8];	// ☓️️ خطای کامپایل
Stack[] t = new Stack[8];			// ✓ بدون خطای کامپایل
 استفاده از انواع داده اولیه (primitives) به عنوان پارامتر نوع ممکن نیست
Stack<int> s;	// ☓️️️ خطای کامپایل
 در صورت تعیین پارامتر نوع، امکان instanceof برای نوع عام وجود ندارد
if(o instanceof Stack<String>);		// ☓️️️ خطای کامپایل
if(o instanceof Stack);			// ✓ بدون خطای کامپایل
...
class GenExc<T> extends Exception {}	// ☓️️  syntax error 
...
try {
 ...
 //..
} catch (ArrayIndexOutOfBoundsException e) { //☓️️ ارور: زیرا تمامی استثناها توسط بلوک بالا دریافت و به این بلوک نمی‌رسد
 //..
}
...
class Child extends Parent{
 void f()throws Exception{} //☓️ زیرا کلاس والد هیچ استثناء مدیریت نکرده ولی فرزند استثناء مدیریت کرده است
}
مثال۲:
class Parent{
...
class Child extends Parent{
 void f() throws ArithmeticException, IOException{} //☓️️ مدیریت دو نوع استثنا
}
مثال۳:
class Parent{
...
class Child extends Parent{
 void f() throws Exception{} // ☓️️ اینجا استثناء از نوع عام‌تر مدیریت شده است
}
مثال۴:
class Parent{
...
if(x==3)
 throw new NullPointerException();
if(x==3)
 throw new IOException();	//☓️️ اینجا سبب ارور می‌شود
...
## 🅱️ مفاهیم
 Business Code : متن اصلی کد برنامه‌ها
 Test Code : کدهایی که برای آزمانیش برنامه‌ها نوشته شوند
...
fail()			// اگر اجزای تست به آن برسد آنگاه آزمون شکست می‌خورد
			// و تنها در خطی از این تابع استفاده می‌کنیم که نمی‌خواهیم اجرای برنامه به هیچ‌وجه به اون خط برسد
و غیره
 متد Before
