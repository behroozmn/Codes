...
- هنگامی که بخواهیم یک mergeRequest را تایید و merge کنیم
    - گزینه Squash commit سبب ترکیب چندین کامیت(در برنچ مد نظر) به یک کامیت در برنچ مقصد می‌شود
        - گزینه Approved به معنای تایید یک درخواست است. یعنی فرد بررسی‌کننده تغییرات کد را مطالعه و ارزیابی کرده و هیچ مشکلی در آن پیدا نکرده است.[در پروژه‌ها، قبل از ادغام، نیاز است که یک یا چند نفر تغییرات را بررسی و تأیید کنند که این فرآیند به عنوان "Code Review" شناخته می‌شود]
-
...
**سوییچ [--abort]** حذف عملیات مرج در زمانی که یک کانفلیکتی رخ داده است
