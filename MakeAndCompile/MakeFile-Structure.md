...
| `wildcard`   | یافتن فایل        | یافتن فایل‌هایی که با الگوی مشخصی همخوانی دارند   | `$(wildcard pattern)`                  | `$(wildcard *.c)`<br>`$(wildcard src/*.c)`                       | `main.c utils.c`<br>`src/main.c src/sum.c src/mine.c` | از فایل‌های واقعی روی دیسک استفاده می‌کند.             |
