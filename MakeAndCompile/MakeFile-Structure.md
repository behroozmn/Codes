...
| `filter`     | فیلتر کردن                            | فیلتر عناصری که باالگوی مشخصی همخوانی‌دارند    | `$(filter pattern...,text)`            | `$(filter a% b%,apple banana cherry)`                            | `apple banana`                                                  | فقط عناصر مطابق با الگو را برمی‌گرداند.                        |
...
| `origin`     | تعریف متغیر درکجا<br>سورس‌متغیر کجاست | تشخیصsourceیک‌متغیر(Environment،File،خط‌فرمان) | `$(origin variable)`                   | `$(origin CC)`                                                   | `default`                                                       | برای بررسی وضعیت متغیرهای سیستمی.<br>bash: `make CFLAGS=-Wall` |
...
| `value`      | مقدار بدون expand                     | بازگرداندن مقدار یک متغیر بدونxpand کردن       | `$(value variable)`                    | `$(value VAR)`                                                   | `$(CC)`                                                         | وقتی می‌خواهید مقدار raw بگیرید.                               |
...
| `basename`   | حذف پسوند                             | حذف پسوندفایل                                  | `$(basename path)`                     | `$(basename file.txt)`                                           | `file`                                                          | اگر پسوند وجود نداشته باشد، همان ورودی را برمی‌گرداند.         |
