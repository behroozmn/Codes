...
| `wildcard`   | یافتن فایل        | یافتن فایل‌هایی که با الگوی مشخصی همخوانی دارند   | `$(wildcard pattern)`                  | `$(wildcard *.c)`<br>`$(wildcard src/*.c)`                       | `main.c utils.c`                                            | از فایل‌های واقعی روی دیسک استفاده می‌کند.             |
...
| `foreach`    | حلقه روی لیست     | اجرای یک دستور برای هر عنصر از یک لیست            | `$(foreach var,list,text)`             | `$(foreach x,$(list),$(x)_done )`                                | `a_done b_done c_done`<br>`src/main.c src/sum.c src/mine.c` | برای تولید لیست‌های پویا کاربرد دارد.                  |
